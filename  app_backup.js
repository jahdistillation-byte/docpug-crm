...
function loadOwners() {
  state.owners = LS.get(OWNERS_KEY, []);
}
function saveOwners() {
...
function addStockLineToVisit(visitId, stockId, qty = 1, { snap = true } = {}) {
  const visits = loadVisits();
  const v = visits.find((x) => x.id === visitId);
  if (!v) return false;
...
function removeStockLineFromVisit(visitId, index) {
  const visits = loadVisits();
  const v = visits.find((x) => x.id === visitId);
  if (!v) return false;
...
// =========================
// STOCK UI calc (до box.innerHTML)
// =========================
ensureVisitStockShape(visit);
  ...
  ? stkExpanded
      .map(
        ...
        <div class="fileRow" style="align-items:center;">
          <div class="fileMain">
            <div class="fileName">${escapeHtml(x.name)}</div>
            <div class="fileMeta">${escapeHtml(String(x.qty))} ${escapeHtml(x.unit)} × ${escapeHtml(String(x.price))} грн = <b>${escapeHtml(String(x.lineTotal))} грн</b></div>
          </div>
          <div class="fileActions">
...
function renderDischargeA4(visitId) {
  ...
  if (expanded.length) {
    const servicesText = expanded
      .map((x) => `${x.name}: ${x.qty} × ${x.price} = ${x.lineTotal} грн`)
      .join("\n");

    const totalText = `Разом за послуги: ${total} грн`;

    if (svcEl) svcEl.textContent = servicesText;
...
function initVisitsTabUI() {
  ...
  $("#visitsSearch", page)?.addEventListener("input", () => {
    renderVisitsTab();
  });
}
...
function initPatientUI() {
  ...
  $("#visitFiles")?.addEventListener("change", async () => {
    const input = $("#visitFiles");
    ...
    if (location.protocol === "file:") {
      alert("Відкрий через http://localhost:8080 (не file://), щоб працювало завантаження.");
      input.value = "";
      return;
    }
    ...
    try {
      const filesList = $("#visitFilesList");
      if (filesList) filesList.innerHTML = `<div class="hint">Завантажую файли…</div>`;
