...
function initVisitFilesUI() {
  ...
  document.addEventListener("click", async (e) => {
    const btn = e.target && e.target.closest ? e.target.closest("#visitFilesList [data-action]") : null;
    if (!btn) return;
    ...
    if (action === "detach") {
      // 1) ПЫТАЕМСЯ отвязать на сервере (если эндпоинт уже есть)
      try {
        const file = getFileById(fileId);
        const stored_name = file?.stored_name;

        if (stored_name) {
          const r = await fetch(`/api/visits/${encodeURIComponent(visitId)}/files/detach`, {
            method: "POST",
            credentials: "include",
            headers: { "Content-Type": "application/json", Accept: "application/json" },
          ...
          if (!r.ok || !j || j.ok !== true) {
            console.warn("⚠️ detach endpoint not ready or failed:", r.status, t);
            // fallback local
        ...
        console.warn("⚠️ detach fallback:", err);
        detachFileFromVisit(visitId, fileId);
      }
    ...
    if (action === "delete") {
      ...
      try {
        const res = await fetch("/api/delete_upload", {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json", Accept: "application/json" },
        ...
        if (!res.ok) {
          console.error("API /delete_upload HTTP", res.status, text);
          throw new Error(`HTTP ${res.status}`);
        }
        if (!json || json.ok !== true) {
          console.error("API /delete_upload bad json", json, text);
          throw new Error(json?.error || "Не вдалося видалити файл");
        }
